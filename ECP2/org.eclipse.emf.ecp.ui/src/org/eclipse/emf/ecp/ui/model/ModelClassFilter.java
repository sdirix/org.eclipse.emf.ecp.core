  ...
  public boolean select(Viewer viewer, Object parentElement, Object element)
  {
    ...
    if (element instanceof EClass)
    {
      return ((EClass)element).getName().toLowerCase().contains(searchTerm.toLowerCase())
          || (EPackage.class.isInstance(parentElement) && ((EPackage)parentElement).getName().toLowerCase().contains(searchTerm.toLowerCase()));
    }
    else if (element instanceof EPackage)
    {
      EPackage ePackage = (EPackage)element;
      Object[] children = ((ModelTreeContentProvider)((TreeViewer)viewer).getContentProvider()).getChildren(element);
