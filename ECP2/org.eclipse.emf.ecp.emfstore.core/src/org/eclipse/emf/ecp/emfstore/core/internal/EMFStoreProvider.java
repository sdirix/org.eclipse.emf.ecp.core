...
import org.eclipse.emf.emfstore.client.model.impl.ServerInfoImpl;
...
public class EMFStoreProvider extends DefaultProvider {
	...
	@Override
	public void fillChildren(ECPModelContext context, Object parent,
		...
		if (parent instanceof InternalRepository) {
			EList<ProjectInfo> projectInfos = WorkspaceManager.getInstance().getCurrentWorkspace()
			.getServerInfos().get(0).getProjectInfos();
			for(ProjectInfo projectInfo:projectInfos){
				childrenList.addChild(new EMFStoreProjectWrapper((InternalRepository)parent,projectInfo, WorkspaceManager.getInstance().getCurrentWorkspace()
						.getServerInfos().get(0)));
			}
