...
public class MEMultiLinkControlDropAdapter implements DropTargetListener
{
   ...
   *          MEEditor input
   * @param reference
   *          EReference being shown in the section
   * @param modelElementContext
   *          the {@link EditorModelelementContext}
   */
  public MEMultiLinkControlDropAdapter(EObject me, EReference reference, EditorModelelementContext modelElementContext)
  {
  ...
  public void drop(DropTargetEvent event)
  {
    if (dropee != null)
    {
      new ECPCommand(dropee, modelElementContext.getEditingDomain())
      {
        @Override
  ...
  /**
   * This checks if this source can be dropped on this target (taking also the drop effect into consideration). The most
   * general case is if the target has the appropriate containment reference for source. Also if all elements in drop
   * source come from the same level in tree (have the same container). These cases are handled here. Sub-Classes can
   * override this method, to implement their own conditions.
   * 
   * @param event
   *          drop target event
  ...
  public boolean canDrop(DropTargetEvent event)
  {
    ...
    if (!eReferenceType.isSuperTypeOf(dropee.eClass()) && !eReferenceType.equals(EcorePackage.eINSTANCE.getEObject()))
    {
      return false;
    }
