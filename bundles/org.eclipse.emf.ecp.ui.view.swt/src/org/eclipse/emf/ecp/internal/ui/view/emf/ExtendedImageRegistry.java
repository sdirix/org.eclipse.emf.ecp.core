...
public class ExtendedImageRegistry {
	...
	protected static String itemURLPrefix = EMFEditPlugin.INSTANCE.getImage("full/obj16/Item")
		.toString() + "#";

	protected static String createChildURLPrefix = EMFEditPlugin.INSTANCE.getImage(
		"full/ctool16/CreateChild").toString()
		+ "#";
		...
		if (object instanceof Image) {
			...
			if (result == null) {
				if (object instanceof ImageDescriptor) {
					...
					final ImageDescriptor composedImageDescriptor = new ComposedImageDescriptor(
	...
	protected void handleDisplayDispose() {
		for (final Image image : table.values()) {
			image.dispose();
		}
		table = null;
	}
...
class URLImageDescriptor extends ImageDescriptor {
	...
	public boolean equals(Object that) {
		if (that instanceof URLImageDescriptor) {
			...
			return url.equals(otherURLImageDescriptor.url)
				&& (key1 == null ? otherURLImageDescriptor.key1 == null : key1
					.equals(otherURLImageDescriptor.key1))
				&& (key2 == null ? otherURLImageDescriptor.key2 == null : key2
					.equals(otherURLImageDescriptor.key2));
		} else {
			return false;
		}
	}
	...
	public Image createImage(boolean returnMissingImageOnError, Device device) {
		// Don't call super because it loses the transparency information if we write this image to
		// disk.
		//
	...
	public InputStream generateGIF(String key1, String key2) {
		...
		try {
			...
			for (int j = 0; j < result; ++j) {
				if (j == tableOffset1 || j == tableOffset1 + 3 || j == tableOffset1 + 6
					|| j == tableOffset1 + 9) {
				...
				if (info2 != null) {
					if (j == tableOffset2 || j == tableOffset2 + 3 || j == tableOffset2 + 6
						|| j == tableOffset2 + 9) {
	...
	protected static class ColorInformation {
		public static ColorInformation getColor(int index) {
			...
			while (entries.size() <= index) {
				...
				entry.factor = new double[] { instance.factor[0], instance.factor[1],
					instance.factor[2], instance.factor[3] };
