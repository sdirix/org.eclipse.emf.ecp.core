...
public class MigrateHandler extends AbstractHandler {
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        while (eAllContents.hasNext()) {
            ...
            List<EReference> bottomUpPath = Helper.getReferencePath(control.getTargetFeature()
                    .getEContainingClass(), childParentReferenceMap);
            // control.getPathToFeature().addAll(bottomUpPath);
            control.getPathToFeature().clear();
            editingDomain.getCommandStack().execute(
                    AddCommand.create(editingDomain, control,
                            ViewPackage.eINSTANCE.getControl_PathToFeature(), bottomUpPath));
        }
        return null;
    }
