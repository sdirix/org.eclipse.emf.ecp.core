...
public class ExpectedValueControl extends SingleControl {
	...
	protected void fillControlComposite(Composite composite) {
		...
		Button bSelectObject = new Button(composite, SWT.PUSH);
		...
		bSelectObject.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				EAttribute attribute = condition.getAttribute();
				if (attribute == null) {
				...
				if (Enum.class.isAssignableFrom(attribuetClazz)) {
					...
					ListDialog ld = new ListDialog(text.getShell());
				...
				if (object != null) {
					EditingDomain editingDomain = AdapterFactoryEditingDomain.getEditingDomainFor(condition);
					editingDomain.getCommandStack().execute(
						SetCommand.create(editingDomain, condition,
							ViewPackage.eINSTANCE.getLeafCondition_ExpectedValue(), object));
