...
//		Object object=getBusinessObjectForPictogramElement(createDiagram);
		...
		for(Object rootObject:project.getElements()){
			...
			int maxWidth=createEObjectAddContext(createDiagram,(EObject)rootObject,currentXOffset,0);
		...
		for(Object rootObject:project.getElements()){
			...
			for(EObject crossReference:eObject.eCrossReferences()){
				ContainerShape shapeFrom=(ContainerShape)getFeatureProvider().getPictogramElementForBusinessObject(eObject);
				ContainerShape shapeTo=(ContainerShape) getFeatureProvider().getPictogramElementForBusinessObject(crossReference);
				AddConnectionContext referenceConnection=new AddConnectionContext(shapeFrom.getAnchors().get(0), shapeTo.getAnchors().get(0));
	...
	private ContainerShape getContainerShape(EObject eObject){
		PictogramElement[] fromShapes=getFeatureProvider().getAllPictogramElementsForBusinessObject(eObject);
		ContainerShape shapeFrom=null;
		for(int i=0;i<fromShapes.length;i=i+2){
	...
	private int createEObjectAddContext(Diagram createDiagram, EObject rootObject, int widthOffset, int heightOffset) {
		...
		int maxWidth=rootObject.eContents().size()*(DEFAULT_OFFSET_X+ContainerSizes.eObjectWidth);
		if(maxWidth==0){
			maxWidth=ContainerSizes.eObjectWidth;
		}
		for(EObject eObejct:rootObject.eContents()){
			int thisMaxWidth=createEObjectAddContext(createDiagram, eObejct,currentXOffset,DEFAULT_OFFSET_Y+heightOffset+ContainerSizes.eObjectHeight );
			int nextXOffset=ContainerSizes.eObjectWidth+DEFAULT_OFFSET_X;
			...
			AddConnectionContext containmentConnection=new AddConnectionContext(shapeFrom.getAnchors().get(0), shapeTo.getAnchors().get(0));
