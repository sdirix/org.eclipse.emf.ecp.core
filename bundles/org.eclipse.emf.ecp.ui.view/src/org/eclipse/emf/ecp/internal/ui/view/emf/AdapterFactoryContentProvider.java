...
public class AdapterFactoryContentProvider implements ITreeContentProvider, INotifyChangedListener {
     ...
     * implement the various IItemContentProvider interfaces. If the adapter factory is an
     ...
     * the call to an object that implements
    ...
    public static class ViewerRefresh implements Runnable {
         ...
         * <code>ViewerRefresh</code>. Duplicative notifications will not be queued.
         * 
         * @param notification
         *            the notification to add to the queue
         * @return whether the queue has been made non-empty, which would indicate that the
         *         <code>ViewerRefresh</code> needs to be {@link Display#asyncExec scheduled} on the
         *         event queue
         * @since 2.2.0
         */
        public synchronized boolean addNotification(IViewerNotification notification) {
            ...
            if (viewer instanceof StructuredViewer) {
                for (Iterator<IViewerNotification> i = notifications.iterator(); i.hasNext()
                        && notification != null;) {
        ...
        protected void refresh(IViewerNotification notification) {
            ...
            if (viewer instanceof StructuredViewer) {
                ...
                AbstractTreeViewer treeViewer = structuredViewer instanceof AbstractTreeViewer ? (AbstractTreeViewer) structuredViewer
                        : null;
                List<Object> expandedElements = treeViewer == null ? Collections.emptyList()
                        : Arrays.asList(treeViewer.getExpandedElements());
                boolean isStaleExpandedElements = AdapterFactoryEditingDomain
