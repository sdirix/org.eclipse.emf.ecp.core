 ...
 * 
 */
public class ToggleAutomaticPreviewUpdateHandler extends AbstractHandler implements IElementUpdater {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		final ISourceProviderService sourceProvServ = (ISourceProviderService) HandlerUtil.getActiveWorkbenchWindow(
			event).getService(ISourceProviderService.class);
		final SessionSourceProvider sessionSourceProvider = (SessionSourceProvider) sourceProvServ
			.getSourceProvider(SessionSourceProvider.SESSION_STATE);
		sessionSourceProvider.setEnabled((Boolean) state.getValue());
	...
	public void updateElement(UIElement element, @SuppressWarnings("rawtypes") Map parameters) {
		...
		final boolean stateValue = (Boolean) state.getValue();
		element.setChecked(stateValue);
		// configure automatic update for the PreviewView
		...
		if (viewPart != null && viewPart instanceof PreviewView) {
			((PreviewView) viewPart).setUpdateAutomatic(stateValue);
		}
	}
}
