...
public class ChildrenDescriptorExtensionTest {
	...
	private static final int TABLECONTROL_CHILD_COUNT = RENDERABLE_CHILD_COUNT + 1 - 3;
	...
	// TODO: Should be not -7, labels, custom control, groupedgrid, span, 2 rules ,separator, table are missing
	private static final int GROUP_CHILD_COUNT = COMPOSITECOLLECTION_CHILD_COUNT - 8;
	...
	// TODO: Should be not -4, labels, group, custom composites, separator, table are missing
	private static final int GRIDEDGROUPROW_CHILD_COUNT = NUMBER_OF_COMPOSITES - 5;
	...
	public void testTableControlChildDescriptors() {
		final int size = getChildrenSize(TablePackage.eINSTANCE.getTableControl());
		assertEquals(TABLECONTROL_CHILD_COUNT, size);
	}

	@Test
	public void testTableColumnChildDescriptors() {
		final int size = getChildrenSize(TablePackage.eINSTANCE.getTableColumn());
